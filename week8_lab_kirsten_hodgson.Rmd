---
title: "Kirsten's Week 8 Code-Along Verion (Multiple Linear Regression"
...
In this week's lab, we are going to perform, assess, interpret, and communicate outcomes from multiple linear regression using the **penguins** data from the `palmerpenguins` package.

### 2. Set-up

See set-up chunk for required packages, etc.
...
- `View(penguins)` to view the entire data frame - remember to do this in the **Console** instead of as a stored code in your .Rmd
...
Now, we'll do some multivariate exploration using a new package, `GGally`
...
That is **WAY** too much. To narrow it down a bit (for example only the continuous variables, which are the variables we're usually most concerned about linearity between re:assumptions), we can select just `species` and the four continuous variables (`bill_length_mm` through `body_mass_g` in the data frame), then pipe right into `ggpairs()`, updating aesthetics within it like we would in `ggplot()`:
...
The resulting matrix gives us a LOT of information about distributions within groups (the histograms in column 1, boxplots in row 1, and density plots along the diagonal), and relationships (the scatterplots below the diagonal, and the correlation values above the diagonal).

Most importantly, it looks like there aren't any notable **non-linear** relationships existing within species between any of the continuous variables (in the scatterplots).
...
There's a lot there - let's break down some of the major pieces. We'll focus on the output starting with the **Coefficients** section (above that is just telling you the model variables and some not-super-useful quantiles for the residuals, which we'll explore later on).
...
- **flipper_length_mm:** The coefficient of flipper length (`r round(penguin_lm_tidy$estimate[2], 1)`) is the average change we expect to see in body mass (grams) for a 1 mm increase in flipper length.
...
What else is included in that output? The standard error is a measure of the accuracy of each coefficient estimate; the t-value is the test statistic comparing the *null hypothesis that the coefficient = 0* to the estimated value of the coefficient; the *p*-value is the probability of finding a coefficient *at least that different from 0* by random chance if the null hypothesis (coefficient = 0) is true.

Excluding the meaningless intercept here, that means that only the coefficient for **speciesChinstrap** is not significantly different from 0 (i.e., if all other variables are constant, chinstrap mean mass does not differ significantly from Adélie mean mass) - which is consistent with exploratory analyses that show very similar body masses for those two species. 
...
Because there is a lot of information to include when reporting the results of multiple linear regressions, results are most often reported in a table. They are also often challenging to create manually, though `broom::tidy()` provides a good starting point.
...
*Note:* If you use this method, the caption should contain the overall model fil information (e.g. R^2^ and model *p*-value).
